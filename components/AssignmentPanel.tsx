...
const PRESET_ROLES = ['Umpire', 'Lunch Provider', 'Scorer', 'Team Manager', 'First Aid']
...
export default function AssignmentPanel({ eventId, members }: Props) {
  ...
  async function addAssignment() {
    ...
    const { data } = await supabase
      .from('assignments')
      .insert({ event_id: eventId, member_id: selectedMember, role })
      .select('*, member:members(id, name)')
      .single()
        ...
        {assignments.map((a) => (
          <div key={a.id} className="flex items-center justify-between bg-gray-50 rounded-lg px-3 py-1.5">
            <div>
              ...
              <span className="text-xs text-gray-500 ml-2">{a.member?.name}</span>
            </div>
